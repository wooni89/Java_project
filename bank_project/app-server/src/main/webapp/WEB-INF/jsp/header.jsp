...
<div style='height:60px;background-color:white;'>
    ...
    <a href='/app/customer/list'>회원</a>
    <a href='/app/account/list'>계좌업무</a>

<c:choose>
    <c:when test="${not empty sessionScope.loginUser}">
        <c:set var="account" value="${accountDao.findByAccountAndOwner(sessionScope.loginUser.no)}"/>
        <c:if test="${not empty account}">
            <a href="/app/transaction/form?accNum=${account.accNum}">은행업무</a>
        </c:if>
    </c:when>
</c:choose>

    <a href='/app/board/list?category=1'>고객VOC</a>
...
<c:choose>
    ...
    <c:otherwise>
        ...
        ${sessionScope.loginUser.name} <a href='/app/auth/logout'>로그아웃</a>
    </c:otherwise>
</c:choose>
