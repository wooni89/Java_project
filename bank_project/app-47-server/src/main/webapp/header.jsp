...
<div style='height:60px;background-color:white;'>
    ...
    <a href='/customer/list.jsp'>회원</a>
    <a href='/account/list.jsp'>계좌업무</a>

<c:choose>
    <c:when test="${not empty sessionScope.loginUser}">
        <c:set var="account" value="${accountDao.findByAccountAndOwner(loginUser.no)}"/>
        <c:if test="${not empty account}">
            <a href="/transaction/form.jsp?accNum=${account.accNum}">은행업무</a>
        </c:if>
    </c:when>
</c:choose>
    <a href='/board/list.jsp?category=1'>고객VOC</a>
