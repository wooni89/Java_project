...
public class MemberHandler implements Handler {
  ...
  public void viewAccount() { // 계좌정보 확인
    ...
    while (true) {
      ...
      Member m = (Member) this.list.retrieve(new Member(accnum, pwd));
  ...
  public void deleteAccount() { // 계좌삭제
    while (true) {
      ...
      Object account = this.list.remove(this.prompt.inputString("삭제할 계좌번호를 입력하세요 : "));

      if (account.equals(account)) {
        System.out.println("계좌가 정상적으로 삭제 되었습니다. 이용해주셔서 감사합니다.");
      } else {
        System.out.println("번호가 다릅니다 다시 입력해주세요.");
      }
